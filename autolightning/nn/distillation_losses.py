...
class SoftDistillationLoss(nn.Module):
    ...
    def forward(self, student_logits, teacher_logits, labels):
        ...
        soft_targets_loss = (
            torch.sum(soft_targets * (soft_targets.log() - soft_prob)) / soft_prob.size()[0] * (self.T**2)
        )
...
class CosineDistillationLoss(nn.Module):
    ...
    def forward(
        ...
        hidden_rep_loss = self.cosine_loss(
            student_hidden_representation,
            teacher_hidden_representation,
            torch.ones(student_hidden_representation.size(0)).to(student_hidden_representation.device),
        )
